...
export default function MerchantProductsPage() {
  ...
  const [filterStatus, setFilterStatus] = useState<"all" | "LIVE" | "PENDING_REVIEW">("all");
  ...
  const filteredProducts = useMemo(() => {
    ...
    return productList.filter((p) => p.offer_status === filterStatus);
    ...
    <div className="mx-auto max-w-7xl px-4 py-8">
      ...
      <div className="mb-6 flex gap-2">
        {(["all", "LIVE", "PENDING_REVIEW"] as const).map((status) => (
          ...
          >
            {status === "all" ? "All" : status === "LIVE" ? "Live" : "Pending Review"}
            <span className="ml-1.5 text-xs opacity-80">
              ({status === "all" ? productList.length : productList.filter((p) => p.offer_status === status).length})
            </span>
          </button>
        ))}
      </div>
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-sm text-muted-foreground">
              {filterStatus === "all" ? "Sync your Shopify store to import products" : "No products with this status"}
            </p>
          </div>
        ) : (
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-border bg-muted/50">
                  ...
                  <th className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wide text-muted-foreground">Brand</th>
              ...
              <tbody className="divide-y divide-border">
                {filteredProducts.map((product) => (
                  <tr key={product.id} className="hover:bg-muted/30">
                    ...
                    <td className="px-6 py-4 text-sm text-foreground">{product.brand_name}</td>
